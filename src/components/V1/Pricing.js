...
const Pricing = ({ className = "" }) => {
  ...
  return (
    <section
      className={`self-stretch flex flex-row items-start justify-center pt-0 px-5 pb-[76.5px] box-border max-w-full text-center text-[65px] text-[#232a42] font-montserrat lg:pb-[50px] lg:box-border mq750:pb-8 mq750:box-border ${className}`}
    >
      <div className="w-[1440.4px] flex flex-col items-start justify-start gap-[77.4px] max-w-full shrink-0 gap-[19px] gap-[39px]">
        ...
        <div className="self-stretch flex flex-row flex-wrap items-start justify-center gap-[38.9px] max-w-full text-left text-[20px]-4 text-[#2da884] gap-[19px]">
          {contentData.plans.map((plan, index) => (
            <div
              key={index}
              className="flex-1 shadow-[17.5px_14.6px_34.03px_rgba(0,_0,_0,_0.09)] flex flex-col items-center justify-center min-w-[283px] max-w-full"
            >
              <BasicPlan basicPlan={plan.name} />
              <div className="self-stretch bg-[#fff] border-[#2da884] border-b-[1px] border-solid overflow-hidden flex flex-col items-start justify-start pt-[38.9px] px-[38px] pb-[37px] text-center text-[65px]">
                <div className="self-stretch flex flex-col items-center justify-center gap-[15.6px]">
                  <div className="self-stretch relative leading-[70px] font-semibold whitespace-nowrap mq1050:text-[52px] mq1050:leading-[56px] mq450:text-[40px] mq450:leading-[42px]">
                    {plan.price}
                  </div>
                  <div className="self-stretch relative text-[15.6px] leading-[24px] font-medium font-plus-jakarta-sans">
                    {plan.period}
                  </div>
                </div>
              </div>
              <div className="self-stretch rounded-t-none rounded-b-[15.56px] bg-[#fff] overflow-hidden flex flex-col items-start justify-start py-[38.9px] px-[38px] text-[15.6px] font-plus-jakarta-sans mq450:pt-[25px] mq450:pb-[25px] mq450:box-border">
                <div className="self-stretch flex flex-col items-center justify-center gap-[38.9px] gap-[19px]">
                  <div className="self-stretch flex flex-col items-start justify-start pt-0 px-0 pb-0 gap-[15.6px]">
                    {plan.features.map((feature, idx) => (
                      <div
                        key={idx}
                        className="self-stretch flex flex-row items-center justify-start gap-[15.6px]"
                      >
                        <img
