...
const Services = ({ className = "" }) => {
  ...
  return (
    <section
      className={`self-stretch flex flex-row items-start justify-center pt-0 px-5 pb-[81.5px] box-border max-w-full text-left text-[65px] text-[#232a42] font-montserrat mq1050:pb-[53px] mq1050:box-border mq750:pb-[34px] mq750:box-border ${className}`}
    >
      <div className="w-[1440.4px] flex flex-row items-center justify-center gap-[77.5px] max-w-full gap-[19px] gap-[39px]">
        <div className="w-[1440.8px] flex flex-col items-start justify-start gap-[23.3px] max-w-full">
          <div className="self-stretch relative leading-[70px] font-semibold mq1050:text-[52px] mq1050:leading-[56px] mq450:text-[40px] mq450:leading-[42px]">
            ...
            <span className="text-[#2da884]">
              {contentdata.title.highlighted}
            </span>
          </div>
        ...
        <div className="self-stretch flex flex-row flex-wrap items-start justify-start gap-x-[38px] gap-y-[36.9px] min-h-[600px] max-w-full text-[27.2px] text-[#fff] gap-[18px]">
          {contentdata.services.map((service, index) => (
            <div
              key={index}
              className="w-[377px] shadow-[17.5px_14.6px_34.03px_rgba(0,_0,_0,_0.09)] rounded-[15.56px] bg-[#2da884] overflow-hidden shrink-0 flex flex-col items-start justify-start py-[38.9px] px-[23px] box-border gap-[15.5px] max-w-full"
            >
              ...
              <div className="self-stretch relative text-[15.6px] leading-[23.33px] font-medium font-plus-jakarta-sans">
                {service.description}
              </div>
            </div>
          ))}
